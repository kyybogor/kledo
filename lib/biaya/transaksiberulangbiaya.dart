...
class TransaksiBerulangBiaya extends StatefulWidget {
  const TransaksiBerulangBiaya({super.key});

  @override
  State<TransaksiBerulangBiaya> createState() => _TransaksiBerulangBiayaState();
}

class _TransaksiBerulangBiayaState extends State<TransaksiBerulangBiaya> {
...
Future<void> fetchInvoices() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.1.21/connect/JSON/index.php'));

      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);

        invoices = data
            .where((item) => item["status"] == "Transaksi Berulang")
